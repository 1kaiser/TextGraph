...
export class TextAsGraph {
  private rectSel;
  private textSel;
  private wordLabelSel;  // Added for word labels
  private inputNode;
  private rectData;
  ...
  constructor() {
    ...
    // Create rectangles for word backgrounds
    ...
    // Create word labels inside rectangles
    this.wordLabelSel = svgSel.appendMany('text', this.rectData)
      .at({ 
        y: height / 2, 
        dy: '.33em', 
        textAnchor: 'middle', 
      ...
      .translate(padding, 0)
      .each(function (d) { d.wordLabelSel = d3.select(this); });

    // Create arrows between words
  ...
  private render() {
    ...
    this.wordLabelSel.at({ opacity: 0 });
    ...
    words.forEach(d => {
      ...
      // Update word label
      this.rectData[d.i].wordLabelSel
        .at({ opacity: 1, x: x + width / 2 })
        .text(d.word);
    ...
    // Hide the input text (but keep it for interaction)
    this.inputNode.style.color = 'transparent';
  ...
  private hover(i?: number, j?: number) {
    ...
    // Highlight rectangles and arrows
