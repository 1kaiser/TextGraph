...
if (!d3.selection.prototype.appendMany) {
  d3.selection.prototype.appendMany = function (name, data) {
    return this.selectAll(name).data(data).enter().append(name.split('.')[0]);
  };
}
if (!d3.selection.prototype.translate) {
  d3.selection.prototype.translate = function (x, y) {
    return this.attr('transform', "translate(".concat(x, ", ").concat(y || 0, ")"));
  };
}
...
d3.conventions = function (opts) {
  opts = opts || {};
  var sel = opts.sel || d3.select('body');
  ...
  var width = opts.width || sel.node().getBoundingClientRect().width;
  var layers = opts.layers || 'sv';
  var result = {
    sel: sel,
    margin: margin,
    height: height,
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      switch (layer) {
        ...
        case 'd':
          // DIV layer
          var div = sel.append('div').style('position', 'absolute');
          result.layers.push(div);
          break;
        case 'c':
          // Canvas layer
          var canvas = sel.append('canvas').attr('width', width).attr('height', height);
...
var _default = exports.default = d3;
},{"d3":"node_modules/d3/index.js"}],"text-as-graph-v2.ts":[function(require,module,exports) {
"use strict";
...
var TextAsGraph = /** @class */function () {
  function TextAsGraph() {
    ...
    var that = this;
    var inputSel = divSel.append('input').st({
    ...
    var rectGroup = svgSel.append('g').attr('class', 'rects');
    var wordGroup = svgSel.append('g').attr('class', 'words');
    var arrowGroup = svgSel.append('g').attr('class', 'arrows');
    this.rectSel = rectGroup;
    this.wordSel = wordGroup;
    this.arrowSel = arrowGroup;
    this.adjMatSel = this.sel.append('svg').st({
      position: 'absolute',
      top: 200,
      left: 50,
      overflow: 'visible'
  ...
  TextAsGraph.prototype.render = function () {
    ...
    var words = this.inputNode.value.split(' ').map(function (word, i) {
      return {
        word: word,
        i: i
      };
    ...
    wordsWithDimensions.forEach(function (d, i) {
      ...
      wordPositions.push(__assign(__assign({}, d), {
        x: currentX,
        y: currentY,
        width: width,
        line: currentLine
    ...
    wordPositions.forEach(function (d, i) {
      // Add rectangle
      _this.rectSel.append('rect').attr('x', d.x - pad + padding).attr('y', d.y + height / 4).attr('width', d.width + pad * 2).attr('height', height / 2).attr('rx', height / 6).attr('ry', height / 6).attr('fill', "hsl(51, 100%, ".concat(Math.random() * 50 + 50, "%)")).attr('stroke', '#000').attr('stroke-width', 1).attr('data-index', i).on('mouseover', function () {
        return _this.hover(i, i);
      }).on('mouseout', function () {
        return _this.hover();
      });
      // Add word text
      _this.wordSel.append('text').attr('x', d.x + d.width / 2 + padding).attr('y', d.y + height / 2).attr('dy', '.33em').attr('text-anchor', 'middle').attr('fill', '#000').attr('font-family', 'monospace').attr('font-size', fontSize).attr('pointer-events', 'none').text(d.word);
    });
    // Draw arrows with simple straight routing
    wordPositions.forEach(function (d, i) {
      if (i < wordPositions.length - 1) {
        var nextWord = wordPositions[i + 1];
        // Calculate connection points at word rectangle edges
        var fromX = d.x + d.width + pad * 2 + padding;
        var fromY = d.y + height / 2;
        var toX = nextWord.x - pad + padding;
        var toY = nextWord.y + height / 2;
        // Use simple straight arrows for all connections
        _this.arrowSel.append('text').attr('x', (fromX + toX) / 2).attr('y', (fromY + toY) / 2).attr('dy', '.33em').attr('text-anchor', 'middle').attr('fill', blue).attr('font-size', 30).attr('data-from', i).attr('data-to', i + 1).text('→');
      }
    });
    // Center the wrapped graph horizontally
    var screenWidth = window.innerWidth;
    var maxLineWidth = Math.max.apply(Math, Array.from(new Set(wordPositions.map(function (w) {
      return w.line;
    }))).map(function (lineNum) {
      var lineWords = wordPositions.filter(function (w) {
        return w.line === lineNum;
      });
      var lastWord = lineWords[lineWords.length - 1];
      return lastWord.x + lastWord.width + pad * 2;
    }));
    var centerOffset = (screenWidth - maxLineWidth) / 2;
    // Apply translation to center all SVG elements
    this.rectSel.attr('transform', "translate(".concat(centerOffset, ", 0)"));
    this.wordSel.attr('transform', "translate(".concat(centerOffset, ", 0)"));
    this.arrowSel.attr('transform', "translate(".concat(centerOffset, ", 0)"));
  ...
  TextAsGraph.prototype.makeAdjMat = function (words) {
    ...
    this.adjMatSel.selectAll('rect').data(pairs).enter().append('rect').attr('width', w).attr('height', w).attr('transform', function (d) {
      return "translate(".concat(d[0].i * w, ", ").concat(d[1].i * w, ")");
    }).attr('fill', function (d) {
      return _this.isEdge(d) ? blue : 'white';
    }).attr('stroke', '#aaa').attr('stroke-width', .2).on('mouseover', function (d) {
      return _this.hover(d[0].i, d[1].i);
    }).on('mouseout', function (d) {
      return _this.hover();
    });
    // Center adj matrix using full screen width
    var screenWidth = window.innerWidth;
    var matrixWidth = w * words.length;
    var matrixLeftOffset = Math.max(0, (screenWidth - matrixWidth) / 2);
    this.adjMatSel.st({
      left: matrixLeftOffset + 'px'
    });
    // Add top words (rotated)
    this.adjMatSel.selectAll('text.top').data(words).enter().append('text').attr('class', 'top').attr('transform', function (d) {
      return "translate(".concat(d.i * w + w / 2, ", -5) rotate(-90)");
    }).attr('text-anchor', 'start').text(function (d) {
      return d.word;
    });
    // Add side words
    this.adjMatSel.selectAll('text.side').data(words).enter().append('text').attr('class', 'side').attr('transform', function (d) {
  ...
  TextAsGraph.prototype.hover = function (i, j) {
    ...
    this.adjMatSel.selectAll('rect').attr('fill', function (d) {
      return !_this.isEdge(d) ? 'white' : d[0].i === i && d[1].i === j ? blueDark : blue;
    });
    // Highlight the text on the adj mat
    var highlightColor = '#000';
    this.adjMatSel.selectAll('text.top').attr('fill', function (d) {
      return d.i === i ? highlightColor : 'gray';
    }).style('font-weight', function (d) {
      return d.i === i ? 'bold' : '';
    });
    this.adjMatSel.selectAll('text.side').attr('fill', function (d) {
      return d.i === j ? highlightColor : 'gray';
    }).style('font-weight', function (d) {
      return d.i === j ? 'bold' : '';
    });
    // Highlight rectangles
    this.rectSel.selectAll('rect').attr('stroke', function () {
    ...
    // Highlight arrows
    this.arrowSel.selectAll('text').attr('stroke', function () {
...
function initialize() {
  ...
  textAsGraphInstance = new _textAsGraphV.TextAsGraph();
...
function setupTextInput() {
  var textInput = document.getElementById('manual-text-input');
  var updateButton = document.getElementById('update-graph');
...
function computeGATAttention(paragraphText, queryText) {
  ...
  var mathResults = computeAttentionMathematically(paragraphTokens, queryTokens);
  return {
...
function computeAttentionMathematically(paragraphTokens, queryTokens) {
  ...
  // Initialize matrices
  var embeddings = createEmbeddingMatrix(paragraphTokens, queryTokens);
  var attentionScores = createAttentionScoreMatrix(embeddings, queryTokens);
  ...
  console.log("\uD83D\uDCCA Attention range: [".concat(minAttention.toFixed(4), ", ").concat(maxAttention.toFixed(4), "]"));
  return {
    ...
    details: {
      ...
      matrixSize: "".concat(n, "x").concat(n),
      nonZeroElements: attentionMatrix.flat().filter(function (v) {
        return v > 0;
      }).length
    }
  };
...
function createEmbeddingMatrix(paragraphTokens, queryTokens) {
  ...
  queryTokens.forEach(function (token, i) {
    ...
    for (var d = 0; d < 64; d++) {
      var value = Math.sin(seed * (d + 1) * 0.01) * 0.5 + Math.cos(seed * (d + 1) * 0.02) * 0.3 + (paragraphTokens.includes(token) ? 0.2 : 0);
      embedding.push(value);
    }
    embeddings.push(embedding);
  });
  console.log("\u2705 Created ".concat(embeddings.length, " embeddings of dimension ").concat(embeddings[0].length));
  return embeddings;
}
  ...
  for (var i = 0; i < n; i++) {
    ...
    for (var j = 0; j < n; j++) {
      if (i === j) {
        ...
        var dotProduct = 0;
        for (var d = 0; d < embeddings[i].length; d++) {
          dotProduct += embeddings[i][d] * embeddings[j][d];
        }

        // Apply LeakyReLU activation
        var score = Math.max(0.1 * dotProduct, dotProduct);

        // Add positional bias (adjacent words get bonus)
        var positionBias = Math.abs(i - j) === 1 ? 0.3 : 0;
...
function applySoftmaxNormalization(scores) {
  ...
  scores.forEach(function (row) {
    ...
    var exp_scores = row.map(function (score) {
      return Math.exp(score);
    });

    // Compute sum for normalization
    var sum = exp_scores.reduce(function (a, b) {
      return a + b;
    }, 0);

    // Normalize to probabilities and pre-round to 1 decimal place
    var normalizedRow = sum > 0 ? exp_scores.map(function (exp_score) {
      return parseFloat((exp_score / sum).toFixed(1));
...
function applyAttentionColoring(attentionData, queryText) {
  ...
  var minAttention = attentionData.minAttention || 0;
  var maxAttention = attentionData.maxAttention || 1;
  console.log("\uD83D\uDCCA Using precalculated attention range: ".concat(minAttention.toFixed(4), " to ").concat(maxAttention.toFixed(4)));
  ...
  d3.selectAll('#text-as-graph text').each(function (d, i) {
    if (i < nodeMaxAttention.length) {
      ...
      var opacity = attention; // Direct mapping: 0-1 → 0%-100% opacity

      d3.select(this).style('fill', '#2563eb') // Fixed blue color
  ...
  d3.selectAll('#text-as-graph rect').each(function (d, i) {
    if (i < nodeMaxAttention.length) {
      var attention = nodeMaxAttention[i];
      var opacity = attention;
      d3.select(this).style('fill', '#fbbf24') // Fixed yellow color
      .style('stroke', '#2563eb').style('opacity', opacity);
    }
  });
...
function updateExistingMatrix(attentionMatrix, queryTokens, minAttention, maxAttention) {
  ...
  var matrixSvg = d3.select('#text-as-graph').selectAll('svg').filter(function () {
...
function updateOriginalMatrixWithAttention(svg, attentionMatrix, queryTokens, minAttention, maxAttention) {
  ...
  console.log("\uD83D\uDCCA Found ".concat(rects.size(), " rectangles to update"));
  rects.each(function (d, index) {
    // Calculate row and column from rectangle position/index
    var row = Math.floor(index / queryTokens.length);
    var col = index % queryTokens.length;
    if (row < attentionMatrix.length && col < queryTokens.length) {
      var attention = attentionMatrix[row][col];
      ...
      d3.select(this).style('fill', fillColor).style('opacity', opacity).attr('data-attention', attention.toFixed(1)).attr('data-row', row).attr('data-col', col);
      console.log("\uD83D\uDCE6 Updated rect[".concat(row, ",").concat(col, "] attention=").concat(attention.toFixed(3), " opacity=").concat(opacity.toFixed(2)));
    }
  });
...
function updateAnyMatrixRects(attentionMatrix, queryTokens, minAttention, maxAttention) {
  ...
  var allRects = d3.selectAll('#text-as-graph rect');
  console.log("\uD83D\uDCCA Found ".concat(allRects.size(), " total rectangles"));

  // Filter to likely matrix rectangles (smaller, grid-positioned)
  var matrixRects = allRects.filter(function () {
  ...
  console.log("\uD83C\uDFAF Found ".concat(matrixRects.size(), " potential matrix rectangles"));
  if (matrixRects.size() >= queryTokens.length * queryTokens.length) {
    matrixRects.each(function (d, index) {
      ...
      if (row < attentionMatrix.length && col < queryTokens.length) {
        ...
        var opacity = attention === 0 ? 0.1 : 0.2 + 0.8 * attention;
        var fillColor = attention === 0 ? '#f3f4f6' : '#3b82f6';
        d3.select(this).style('fill', fillColor).style('opacity', opacity).attr('data-attention', attention.toFixed(1));
      }
    });
  }
}
...
function addAttentionScoresToMatrix(svg, attentionMatrix, queryTokens, minAttention, maxAttention) {
  ...
  rects.each(function (d, index) {
    ...
    var col = index % queryTokens.length;
    if (row < attentionMatrix.length && col < queryTokens.length) {
      var _rect$attr, _rect$attr2;
      var attention = attentionMatrix[row][col];

      // Get rectangle position
      var x = parseFloat(rect.attr('x') || ((_rect$attr = rect.attr('transform')) === null || _rect$attr === void 0 || (_rect$attr = _rect$attr.match(/translate\((\d+)/)) === null || _rect$attr === void 0 ? void 0 : _rect$attr[1]) || 0);
      var y = parseFloat(rect.attr('y') || ((_rect$attr2 = rect.attr('transform')) === null || _rect$attr2 === void 0 || (_rect$attr2 = _rect$attr2.match(/translate\(\d+,\s*(\d+)/)) === null || _rect$attr2 === void 0 ? void 0 : _rect$attr2[1]) || 0);
      var width = parseFloat(rect.attr('width')) || 20;
      var height = parseFloat(rect.attr('height')) || 20;

      // Add attention score text
      svg.append('text').attr('class', 'gat-attention-score').attr('x', x + width / 2).attr('y', y + height / 2).attr('dy', '.35em').attr('text-anchor', 'middle').style('font-size', '10px').style('font-weight', 'bold').style('fill', attention > 0.5 ? 'white' : '#1f2937').style('pointer-events', 'none').text(attention.toFixed(1));
    }
  });
}
...
function updateMatrixInMainSvg(svg, attentionMatrix, queryTokens, minAttention, maxAttention) {
  ...
  var matrixSize = 39; // 30 * 1.3 = 39
  var matrixStartX = 50;
  var matrixStartY = 200;

  // Add matrix title
  svg.append('text').attr('class', 'matrix-label').attr('x', matrixStartX + queryTokens.length * matrixSize / 2).attr('y', matrixStartY - 20).attr('text-anchor', 'middle').style('font-size', '14px').style('font-weight', 'bold').style('fill', '#374151').text('🧮 Attention Matrix');

  // Create matrix cells with GAT attention values
  attentionMatrix.forEach(function (row, i) {
    row.forEach(function (attention, j) {
      ...
      // Create matrix cell
      svg.append('rect').attr('class', 'adj-mat-square').attr('x', matrixStartX + j * matrixSize).attr('y', matrixStartY + i * matrixSize).attr('width', matrixSize - 2).attr('height', matrixSize - 2).style('fill', cellColor).style('opacity', opacity).style('stroke', '#1f2937').style('stroke-width', 1).style('cursor', 'pointer').attr('data-row', i).attr('data-col', j).attr('data-attention', attention);

      // Add attention value text
      svg.append('text').attr('class', 'attention-score').attr('x', matrixStartX + j * matrixSize + matrixSize / 2).attr('y', matrixStartY + i * matrixSize + matrixSize / 2).attr('dy', '.35em').attr('text-anchor', 'middle').style('font-size', '10px').style('font-weight', 'bold').style('fill', opacity > 0.5 ? 'white' : '#1f2937').style('pointer-events', 'none').text(attention.toFixed(1));
    });
  });
  ...
  queryTokens.forEach(function (token, i) {
    ...
    svg.append('text').attr('class', 'matrix-label').attr('x', matrixStartX - 5).attr('y', matrixStartY + i * matrixSize + matrixSize / 2).attr('dy', '.35em').attr('text-anchor', 'end').style('font-size', '10px').style('fill', '#666').text(token.substring(0, 6));

    // Column labels (top)
    svg.append('text').attr('class', 'matrix-label').attr('x', matrixStartX + i * matrixSize + matrixSize / 2).attr('y', matrixStartY - 5).attr('text-anchor', 'middle').style('font-size', '10px').style('fill', '#666').text(token.substring(0, 6));
  });
}
...
function updateMatrixSvg(svg, attentionMatrix, queryTokens, minAttention, maxAttention) {
  ...
  svg.selectAll('.adj-mat-square').each(function (d, i) {
    var row = Math.floor(i / queryTokens.length);
    var col = i % queryTokens.length;
    if (row < attentionMatrix.length && col < queryTokens.length) {
      var attention = attentionMatrix[row][col];
      ...
      d3.select(this).style('fill', cellColor).style('opacity', opacity).attr('data-attention', attention);
    }
  });
}
...
function setupAttentionHoverInteractions(attentionData) {
  ...
  d3.selectAll('#text-as-graph text').on('mouseover', function (d, i) {
    if (i < queryTokens.length) {
      ...
      d3.selectAll('.adj-mat-square').each(function (_, squareIndex) {
        var row = Math.floor(squareIndex / queryTokens.length);
        var col = squareIndex % queryTokens.length;
        if (row === i || col === i) {
      ...
      // Show tooltip
      var attention = attentionMatrix[i] || [];
      var tooltip = "".concat(queryTokens[i], ": [").concat(attention.map(function (a) {
        return a.toFixed(2);
      }).join(', '), "]");
  ...
  d3.selectAll('.adj-mat-square').on('mouseover', function (d, i) {
    ...
    var attention = attentionMatrix[row] && attentionMatrix[row][col];
    if (attention !== undefined) {
      console.log("\uD83D\uDD17 Matrix [".concat(row, ",").concat(col, "]: ").concat(queryTokens[row], " \u2192 ").concat(queryTokens[col], " = ").concat(attention.toFixed(3)));
    }
  }).on('mouseout', function () {
